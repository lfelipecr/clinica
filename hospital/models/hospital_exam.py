...
class HospitalExam(models.Model):
    ...
    consultation_id = fields.Many2one('hospital.consultation', string='Consulta', required=True, ondelete='cascade')
    request_date = fields.Datetime(string='Fecha de Solicitud', default=fields.Datetime.now, required=True)
    exam_type_id = fields.Many2one('hospital.exam.type', string='Tipo de Examen', required=True)
